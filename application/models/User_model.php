...
class User_model extends CI_Model {
                    ...
                    <div class="panel panel-default" style="border-color: #fff;">
                        <div class="panel-body" style="    height: 117px; ">                       
                            <a style="position: absolute;margin: 28px 0px 0px 0px;font-weight: 700;" onclick="window.location = ' . $Tab2Location . ';">
                               No Of New ' . $vials . ' Targeted For ' . $this->getMonthName($this->nextMonth) . "&nbsp" . date('Y') . '
                            </a>
                             <div class="pull-right">
                     ...
                     <div class="panel panel-default" style="border-color: #fff;">
                        <div class="panel-body" style="    height: 117px; ">
                            <a style="position: absolute;margin: 28px 0px 0px 0px;font-weight: 700;" onclick="window.location = ' . $Tab3Location . '">
                                Planning For The Month Of ' . $this->getMonthName($this->nextMonth) . "&nbsp" . date('Y') . ' </a>
                        </div>
                    </div>
                </div>';
    ...
    function PlanningExist($Doctor_Id = "", $month) {
        $this->db->select('*');
        $this->db->from('Rx_Planning');
    ...
    function ActualPriorityExist($Doctor_Id, $month) {
        $this->db->select('*');
        $this->db->from('Actual_Doctor_Priority');
        $this->db->where(array('VEEVA_Employee_ID' => $this->VEEVA_Employee_ID, 'Product_Id' => $this->Product_Id, 'Doctor_Id' => $Doctor_Id, 'month' => $month, 'Year' => $this->nextYear));
        $query = $this->db->get();
        return $query->row_array();
    }
    ...
    function ActivityPlanned($Doctor_Id = "", $month) {
        $this->db->select('*');
