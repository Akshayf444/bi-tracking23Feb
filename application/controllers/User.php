...
//$this->User_model->insertLog($logdata);
    ...
    public function Planning() {
        ...
        $logmessage = array();
        if ($this->is_logged_in('BDM')) {
            $targetSet = $this->User_model->Rx_Target_month($this->VEEVA_Employee_ID, $this->Product_Id, $current_month, $this->nextYear);
            if (!empty($targetSet) && $targetSet->target > 0) {
                $data['result'] = $this->User_model->getPlanning($this->VEEVA_Employee_ID, $this->Product_Id, $current_month, $this->nextYear);
...
//var_dump($doc_id);
                    for ($i = 0; $i < count($this->input->post('value')); $i++) {
                        ...
                        $result = $this->User_model->PlanningExist($doc_id[$i], $current_month);
//var_dump($result);
                        ...
                        if (empty($result)) {
                            ...
                            $this->db->where(array('VEEVA_Employee_ID' => $this->VEEVA_Employee_ID, 'Product_Id' => $this->Product_Id, 'Doctor_Id' => $doc_id[$i], 'month' => $current_month));
            ...
            $data['expected'] = $this->User_model->Expected_Rx($this->VEEVA_Employee_ID, $this->Product_Id, $current_month);
    ...
    public function ActivityPlanning() {
        ...
        $created_at = $cutoffdates[1];
        $this->nextMonth = $current_month;
        $check_planning = $this->User_model->priority_check($this->VEEVA_Employee_ID, $this->Product_Id, $current_month);
            ...
            if ($this->input->post()) {
                for ($i = 0; $i < count($this->input->post('Doctor_Id')); $i ++) {
                    ...
                    if (trim($Activity[$i]) != '') {
                        ...
                        $result = $this->User_model->ActivityPlanned($docid[$i], $current_month);
                        if (empty($result)) {
                            ...
                            if ($this->Product_Id == 4 || $this->Product_Id == 6) {
                                $data2['Product_Id'] = 4;
                                $this->db->where(array('VEEVA_Employee_ID' => $this->VEEVA_Employee_ID, 'Product_Id' => 4, 'Doctor_Id' => $docid[$i], 'Year' => $this->nextYear, 'month' => (int) $current_month));
                                $this->db->update('Activity_Planning', $data2);

                                $data2['Product_Id'] = 6;
                                $this->db->where(array('VEEVA_Employee_ID' => $this->VEEVA_Employee_ID, 'Product_Id' => 6, 'Doctor_Id' => $docid[$i], 'Year' => $this->nextYear, 'month' => (int) $current_month));
                                $this->db->update('Activity_Planning', $data2);
                                ...
                                $this->db->where(array('VEEVA_Employee_ID' => $this->VEEVA_Employee_ID, 'Product_Id' => $this->Product_Id, 'Doctor_Id' => $docid[$i], 'Year' => $this->nextYear, 'month' => (int) $current_month));
                                $this->db->update('Activity_Planning', $data2);
                                array_push($logmessage, 'Activity Planning Updated');
                                array_push($messages, $this->Master_Model->DisplayAlert('Activity Updated Successfully.', 'success'));
                            }
...
//var_dump($doctor_ids);
        ...
        if ($this->input->post()) {
            for ($i = 0; $i < count($this->input->post('priority')); $i++) {
                ...
                $result = $this->User_model->ActualPriorityExist($priority[$i], $current_month);
                $data2 = array(
                    'VEEVA_Employee_ID' => $this->VEEVA_Employee_ID,
                    'Product_Id' => $this->Product_Id,
                    'month' => $current_month,
                ...
                if (empty($result)) {
                    ...
                    $this->db->where(array('VEEVA_Employee_Id' => $this->VEEVA_Employee_ID, 'Product_Id' => $this->Product_Id, 'month' => $current_month, 'Doctor_Id' => $priority[$i]));
...
// if (empty($result)) {
            ...
            for ($i = 0; $i < count($this->input->post('value')); $i++) {
                ...
                $month = $this->User_model->calculateMonth($this->nextMonth, 1);
                ...
                if (empty($result)) {
                    ...
                    $this->message = $this->Master_Model->DisplayAlert('Doctor Priority ' . date('M', strtotime($created_at)) . '' . $this->nextYear . ' has been saved successfully! Thank you!.', 'success');
                } else {
                    ...
                    $this->message = $this->Master_Model->DisplayAlert('Doctor Priority ' . date('M', strtotime($created_at)) . '' . $this->nextYear . ' has been Updated successfully! Thank you!.', 'success');
                }
            }
